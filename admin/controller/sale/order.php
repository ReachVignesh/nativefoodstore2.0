		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				foreach ($products as $product) {
					...
					//Queries below was added to display the weight and weight class in invoice
					$product_weight_query = $this->db->query("SELECT weight, weight_class_id FROM " . DB_PREFIX . "product WHERE product_id = '" . $product['product_id'] . "'");
					...
					//$tax_class_query = $this->db->query("SELECT tcd.unit FROM " . DB_PREFIX . "tax_class tc LEFT JOIN " . DB_PREFIX . "tax_class_description tcd ON (tc.tax_class_id = tcd.tax_class_id) WHERE tcd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND tc.tax_class_id = '" . $product_tax_query->row['tax_class_id'] . "'");
					//$tax_class_query = $this->db->query("SELECT tax_class_id FROM " . DB_PREFIX . "product WHERE product_id = '" . $product['product_id'] . "'");
					$mpn_query = $this->db->query("SELECT mpn FROM " . DB_PREFIX . "product WHERE product_id = '" . $product['product_id'] . "'");
					...
					$product_data[] = array(
						...
						'option'   => $option_data,
						'quantity' => $product['quantity'],
						//Added for displaying weight
						'weight' => $product_weight_query->row['weight'],
						'weight_class' => $weight_class_query->row['unit'],
						'mpn' => $mpn_query->row['mpn'],
						//'tax' => $tax_class_query->row['unit'],
						//'tax'      => $this->currency->format($this->config->get('config_tax') ? ($product['tax']) : 0),
						'price'    => $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
						'total'    => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'])
					);
				}
			...
			if ($order_info && $order_info['shipping_code']) {
				...
				foreach ($products as $product) {
					...
					$product_data[] = array(
						...
						//Above line was added for displaying weight
						//'tax'      => $this->tax->format($product_info['tax'], $this->config->get('config_tax_class_id'))
					);
				}

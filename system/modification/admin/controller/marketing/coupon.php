	...
	protected function getList() {
		...
		$filter_data = array(
			...
			// kupon
					...
					$this->model_marketing_coupon->checkFieldCoupon();

					$data['entry_customer_group'] = $this->language->get('entry_customer_group');
					$data['entry_customer_group_all'] = $this->language->get('entry_customer_group_all');
			//
		...
		foreach ($results as $result) {
			$data['coupons'][] = array(
				...
				// kupon 
				'customer_group' => ($result['customer_group']) ? unserialize($result['customer_group']) : array(),	
				//		
		...
		if (isset($this->request->get['coupon_id']) && (!$this->request->server['REQUEST_METHOD'] != 'POST')) {
			$coupon_info = $this->model_marketing_coupon->getCoupon($this->request->get['coupon_id']);
				...
				if (isset($this->request->post['coupon_customer_group'])) {
					$data['coupon_customer_group'] = $this->request->post['coupon_customer_group'];
				} elseif (!empty($coupon_info)) {	
					$data['coupon_customer_group'] = ($coupon_info['customer_group']) ? unserialize($coupon_info['customer_group']) : array();
				} else {
